...
SELECT 'hr:jrivera', 'jordan.rivera@northwind.example', 'jordan.rivera@example.com', 'Jordan Rivera', groups.id, true
FROM groups WHERE groups.code = 'NWA'
ON CONFLICT (email) DO NOTHING;

INSERT INTO app_users (external_id, email, personal_email, display_name, group_id, firearm_access)
SELECT 'hr:achen', 'avery.chen@northwind.example', 'avery.chen@example.com', 'Avery Chen', groups.id, false
FROM groups WHERE groups.code = 'NWA-ATL'
ON CONFLICT (email) DO NOTHING;

INSERT INTO app_users (external_id, email, personal_email, display_name, group_id, firearm_access)
SELECT 'hr:mhill', 'marcus.hill@northwind.example', 'marcus.hill@example.com', 'Marcus Hill', groups.id, false
...
FROM app_users
CROSS JOIN roles
WHERE app_users.email = 'jordan.rivera@northwind.example'
  AND roles.code = 'asset_manager'
ON CONFLICT DO NOTHING;

-- Additional seeded users so every role is represented for dev role-switch testing.
INSERT INTO app_users (external_id, email, personal_email, display_name, group_id, firearm_access)
SELECT 'hr:dpark', 'dana.park@northwind.example', 'dana.park@example.com', 'Dana Park', groups.id, false
FROM groups WHERE groups.code = 'NWA'
ON CONFLICT (email) DO NOTHING;

INSERT INTO app_users (external_id, email, personal_email, display_name, group_id, firearm_access)
SELECT 'hr:lkim', 'logan.kim@northwind.example', 'logan.kim@example.com', 'Logan Kim', groups.id, false
FROM groups WHERE groups.code = 'NWA-ATL'
ON CONFLICT (email) DO NOTHING;

-- Distinct role per user: asset_manager (Jordan, above), approver (Avery),
-- read_only (Marcus), admin (Dana), office_asset_manager (Logan).
INSERT INTO user_roles (user_id, role_id)
SELECT u.id, r.id
FROM app_users u
CROSS JOIN roles r
WHERE (u.email = 'avery.chen@northwind.example'  AND r.code = 'approver')
   OR (u.email = 'marcus.hill@northwind.example' AND r.code = 'read_only')
   OR (u.email = 'dana.park@northwind.example'   AND r.code = 'admin')
   OR (u.email = 'logan.kim@northwind.example'   AND r.code = 'office_asset_manager')
ON CONFLICT DO NOTHING;
...
WHERE asset_categories.code = 'computers'
  ...
  AND app_users.email = 'avery.chen@northwind.example'
...
WHERE asset_categories.code = 'computers'
  AND groups.code = 'NWA'
  AND app_users.email = 'jordan.rivera@northwind.example'
ON CONFLICT (asset_tag) DO NOTHING;
...
-- the seeded users who have the firearm_access flag (jordan.rivera) plus a few
...
WHERE asset_categories.code = 'firearms' AND groups.code = 'NWA' AND app_users.email = 'jordan.rivera@northwind.example'
ON CONFLICT (asset_tag) DO NOTHING;
...
WHERE subject.email = 'avery.chen@northwind.example'
  AND owner.email = 'jordan.rivera@northwind.example'
...
SELECT 'APR-3381', workflows.id, app_users.id, 'Computer issuance for Avery Chen', 'pending'
FROM workflows
CROSS JOIN app_users
...
JOIN app_users subj  ON subj.email  = 'marcus.hill@northwind.example'
...
SELECT 'APR-3379', w.id, appr.id, 'Laptop transfer to Marcus Hill', 'pending', TIMESTAMPTZ '2026-05-10 14:00:00-04'
FROM workflows w
